AIM:
To write a C program that implements a simple singly linked list and performs basic operations such as:
...
Step 2:
Read data user and store in newnode->data.
Step 3:
Set newnode->next = NULL.
Step 4:
Return the created node.
B) Algorithm to Insert at End
Step 1:
Create a new node.
Step 2:
If head == NULL:
Set head = newnode (first node).
STOP.
Step 3:
Else traverse the list using a temp pointer until temp->next == NULL.
Step 4:
Set temp->next = newnode.
Step 5:
Stop.
C) Algorithm to Display Linked List
Step 1:
If head == NULL:
Print "List is empty".
Stop.
