AIM:
To write a C program that converts a given infix expression (e.g., A+B*C) into a corresponding postfix expression (e.g., ABC*+) using a stack for handling operators and parentheses
ALGORITHM:
Step 1:
Start.
Step 2:
...
Add it directly to the postfix output.
If the symbol is ‘(’ :
...
Step 5:
After scanning entire infix expression, pop all remaining operators from stack and add to postfix.
Step 6:
End.
PROGRAM:
